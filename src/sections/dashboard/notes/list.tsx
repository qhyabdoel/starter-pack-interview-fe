...
export function NoteListView({ title = 'List' }: Props) {
  const [rows, setRows] = useState<Array<Note>>([]);
  ...
  const formAddTitle = 'Add Note';
  const formUpdateTitle = 'Update Note';
  const [formTitle, setFormTitle] = useState(formAddTitle);

  const getNotesFromAPI = async () => {
    const tasksApiRes = await axios.get(endpoints.notes);
    setRows(tasksApiRes?.data || []);
  };

  const handleSubmitForm = async () => {
    console.log({ valId });
    if (valId) await putNoteToAPI();
    else await postNoteToAPI();
    await getNotesFromAPI();
    setOpenFormModal(false);
  };

  const handleSubmitDelete = async () => {
    const tasksApiRes = await axios.delete(`${endpoints.notes}/${valId}`);
  ...
  const postNoteToAPI = async () => {
    const tasksApiRes = await axios.post(endpoints.notes, {
      title: valueTitle,
  ...
  const putNoteToAPI = async () => {
    const tasksApiRes = await axios.put(`${endpoints.notes}/${valId}`, {
      title: valueTitle,
      description: valueDescription,
      status: valueStatus,
    });
  ...
  const handleOpenFormModal = (paramTitle: string, data?: Note) => {
